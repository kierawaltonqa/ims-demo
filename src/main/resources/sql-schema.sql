...
create table if not exists ims.orderline(orderlineID int unique primary key auto_increment, orderID int, itemID int, quantity int, foreign key(itemID) references ims.items(itemID), foreign key(orderID) references ims.orders(orderID));
